...
class ScenarioTest < AcceptanceTest
  ...
  test "scenario-1 of sprint-2" do
    ...
    # 1.4. エンジンを登録する
    fill_in "型式", with: "ENG0002"
    fill_in "S/#", with: "SN0001"
    fill_in "販売型式", with: "ENG9002"
    ...
    within "tbody > tr" do
      ...
      assert_equal "ENG0002", nth_tag(:td, 11).text # 型式
      assert_equal "SN0001", nth_tag(:td, 12).text  # S/#
      assert_equal "整備前", nth_tag(:td, 13).text  # ステータス
    end
    ...
    select "2013", from: "repair[desirable_finish_date(1i)]" # 希望完成日(年)
    select "12月", from: "repair[desirable_finish_date(2i)]" # 希望完成日(月)
    select "23",   from: "repair[desirable_finish_date(3i)]" # 希望完成日(日)
    ...
    click_link "Back"
    within "tbody > tr" do
      assert_equal "2013-12-23", nth_tag(:td, 4).text # 完成希望日
      assert_equal "整備前", nth_tag(:td, 13).text    # ステータス
    end
    ...
    # 3.3. 整備開始画面を開く
    click_link "開始登録"
    select "2013", from: "repair[start_date(1i)]" # 整備開始日(年)
    select "12月", from: "repair[start_date(2i)]" # 整備開始日(月)
    select "15", from: "repair[start_date(3i)]"   # 整備開始日(日)
    comment = "整備がんばります！"
    ...
    click_link "Back"
    within "tbody > tr" do
      assert_equal "2013-12-15", nth_tag(:td, 5).text # 整備開始日
      assert_equal comment, nth_tag(:td, 9).text      # 整備前コメント
      assert_equal "整備中", nth_tag(:td, 13).text    # ステータス
    end
    ...
    # 4.3. 整備完了画面を開く
    click_link "完了登録"
    select "2013", from: "repair[finish_date(1i)]" # 完了日(年)
    select "12月", from: "repair[finish_date(2i)]" # 完了日(月)
    select "20", from: "repair[finish_date(3i)]"   # 完了日(日)
    comment = "整備完了しました！"
    ...
    click_link "Back"
    within "tbody > tr" do
      assert_equal "2013-12-20", nth_tag(:td, 6).text # 整備完了日
      assert_equal comment, nth_tag(:td, 10).text     # 完成品コメント
      assert_equal "完成品", nth_tag(:td, 13).text    # ステータス
    end
    ...
    select "法華倶楽部", from: "設置先"
    comment = "ちょっと調子がおかしいので交換してください。"
    fill_in "コメント(交換理由)", with: comment
    select "2013", from: "engineorder[desirable_delivery_date(1i)]"
    select "12月", from: "engineorder[desirable_delivery_date(2i)]"
    select "31", from: "engineorder[desirable_delivery_date(3i)]"
    ...
    within "tbody > tr" do
      ...
      assert_equal "2013-12-31", nth_tag(:td, 12).text        # 希望納期
    end
